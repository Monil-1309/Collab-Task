...
const columns = [
  ...
  { id: "on-hold", title: "On Hold", color: "bg-orange-100 dark:bg-orange-900/20" },
  { id: "done", title: "Done", color: "bg-green-100 dark:bg-green-900/20" },
  { id: "deployed", title: "Deployed", color: "bg-purple-100 dark:bg-purple-900/20" },
  { id: "cancelled", title: "Cancelled", color: "bg-red-100 dark:bg-red-900/20" },
] as const

export function BoardView({ tasks, searchQuery, filters }: BoardViewProps) {
  const { moveTask, addTask } = useTasks()
  const { draggedItem, handleDragStart, handleDragEnd, handleDragOver, handleDrop } = useDragDrop()
  const [isTaskModalOpen, setIsTaskModalOpen] = useState(false)
  const [selectedColumn, setSelectedColumn] = useState<Task["status"]>("backlog")
      ...
      <div className="flex gap-6 overflow-x-auto pb-6">
        {columns.map((column) => {
          ...
          return (
            <div
              key={column.id}
              className={cn("flex-shrink-0 w-80 rounded-lg border p-4", column.color)}
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, (taskId) => handleTaskDrop(column.id, taskId))}
            >
              ...
              <div className="space-y-3 min-h-[200px]">
                {columnTasks.map((task) => (
                  <div
                    ...
                    className={cn("cursor-move transition-opacity", draggedItem === task.id && "opacity-50")}
                  >
                    <TaskCard task={task} />
                  </div>
                ))}
